...
/*Start Action info fetch one recored*/
		...
		function fetchoneinfo($table,$where){
			...
			return $info;	
                ...
                function fetchoneinfo2($table,$where){
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('*');
			foreach($where as $list =>$key):
		...
		function info_fetch_countmtoallinfos($in,$id,$a,$b)
		{
			...
			$CI->db->from('newmt');
                        if(is_array($in)){
                            $CI->db->where_in('newmt.catofvechicle',$in);
                        }else{
			...
			return $result; 
                ...
                function info_fetch_countmtoallinfos2($in,$id,$a,$b)
		{
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('newmt.*,issue_vehicle.reg_no, issue_vehicle.date_of_duty');
			$CI->db->join('issue_vehicle', 'issue_vehicle.reg_no = newmt.mt_id');
			$CI->db->from('newmt');
                        if(is_array($in)){
                            $CI->db->where_in('newmt.catofvechicle',$in);
			...
			$CI->db->where('issue_vehicle.bat_id', $id);
			$CI->db->where('newmt.bat_id', $id);
			$CI->db->where('issue_vehicle.date_of_duty <=',$a);
	 		//$CI->db->where('issue_vehicle.cyear1',$b);
                        $CI->db->order_by('issue_vehicle.reg_no, issue_vehicle.date_of_duty','desc');
                        $CI->db->limit('18446744073709551615',0);
                        $query1 = $CI->db->get_compiled_select();
                        $CI->db->select('*');
                        $CI->db->from("({$query1}) as x");
	 		$CI->db->group_by('x.reg_no');
			$query = $CI->db->get();	
//                        echo $CI->db->last_query();
			$result = $query->result();
			return $result; 
		...
		}function fetchoneinfopoltotaldeciisssh($id,$m,$y,$ids){
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('*');
			$CI->db->select('SUM(pamo) AS deci');
		...
		function fetchoneinfopollistpost($id,$m,$y,$ids){
			...
			//$CI->db->order_by('issue_vehicle_id','DESC');
                        $CI->db->order_by('date_of_duty','DESC');
			$query = $CI->db->get('issue_vehicle');
//			echo $CI->db->last_query();
			$info = $query->row();
			...
			return $info;
                ...
                function fetchoneinfopollistpost_2($id,$date,$ids){
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('*');
			$CI->db->where_in('reg_no',$id);
			$CI->db->where('date_of_duty <=',$date);
			//$CI->db->where('cyear1',$y);
			$CI->db->where_in('bat_id', $ids);
                        $query = $CI->db->from('issue_vehicle');
			//$CI->db->order_by('issue_vehicle_id','DESC');
                        $CI->db->order_by('issue_vehicle.reg_no,date_of_duty','DESC');
                        $CI->db->limit('18446744073709551615',0);
//                        $CI->db->order_by('issue_vehicle.reg_no, issue_vehicle.date_of_duty','desc');   
                        $query1 = $CI->db->get_compiled_select();
                        $CI->db->select('*');
...
//			echo '<br>'.$CI->db->last_query();
		...
		function fetchoneinfopollistpost8($id,$m,$y){
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('*');
			$CI->db->where('reg_no',$id);
		...
		function fetchoneinfopollistpost9($id,$m,$y){
			$CI= & get_instance();
			$CI->load->database();
			$CI->db->select('*');
			$CI->db->where('reg_no',$id);
		...
		function getBadge($page,$pageErrorCounter){
			if(isset($pageErrorCounter)){
				if(isset($pageErrorCounter[$page]) && ($pageErrorCounter[$page]>0)){
					//$pageErrorCounter[$page]='';
					$data = '<span class="badge-cross"><!--i class="fa fa-times-circle" aria-hidden="true" -->'.$pageErrorCounter[$page].'</span>';
				}else{
					$data = '<span class="badge-ok"><i class="fa fa-check fa-6" aria-hidden="true"></i></span>';
				}
			}else{
				$data = '';
			}
		...
		function getActivePageClass($page,$selected_page){
		...
		function break_name_into_spaces($name){
	...
	function convertDate($date){
		...
		//echo $date;
		return date_format($date,"Y-m-d");//, strtotime($date));
	}
	/**
			convert date from  Y-m-d H:i:s to d/m/Y to
		*/
	function convertDate2($date){
		$date = date_create_from_format("Y-m-d H:i:s",$date);
		//echo $date;
		return date_format($date,"d/m/Y");//, strtotime($date));
	}
	function converDateFromYMDtoDMY($date){
		$date = date_create_from_format("Y-m-d",$date);
		//echo $date;
		return date_format($date,"d/m/Y");//, strtotime($date));
	}
	function convertDateYMDHMStoDMYHMS($date){
		$date = date_create_from_format("Y-m-d H:i:s",$date);
		//echo $date;
		return date_format($date,"d/m/Y H:i:s");//, strtotime($date));
	}
        function convertDateYMDtoDMYHyphen($date_){
		$date = date_create_from_format("Y-m-d",$date_);
		//echo $date;
                if(!($date==false)){
                    return date_format($date,"d-m-Y");//, strtotime($date));
                }else{
